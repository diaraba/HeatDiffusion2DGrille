...
// Initialize MPI if needed
...
#ifdef SEQUENTIAL
    if (rank == 0)
        std::cout << "Using Sequential solver" << std::endl;
...
#ifndef USE_MPI
    if (rank == 0)
        std::cout << "Using OpenMP solver" << std::endl;
...
#ifndef USE_OPENMP
    if (rank == 0)
        std::cout << "Using MPI solver" << std::endl;
    ...
    if (rank == 0)
        std::cout << "Using Hybrid MPI+OpenMP solver" << std::endl;
